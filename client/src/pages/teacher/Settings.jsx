  ...
  return (
    <div>
      ...
      <div className="card p-6 mb-5 max-w-md">
        <h3 className="text-sm font-semibold mb-4">Mi perfil</h3>
        <form onSubmit={saveProfile} className="space-y-3">
          ...
          <button type="submit" className="w-full py-2.5 bg-grad rounded-xl text-white text-sm font-semibold">Guardar</button>
        </form>
      </div>
      ...
      <div className="card p-6 mb-5 max-w-md">
        ...
        <form onSubmit={changePassword} className="space-y-3">
          <input type="password" placeholder="Contraseña actual" value={pwd.current} onChange={(e) => setPwd({ ...pwd, current: e.target.value })}
            className="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm" />
          <input type="password" placeholder="Nueva contraseña" value={pwd.next} onChange={(e) => setPwd({ ...pwd, next: e.target.value })}
            className="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm" />
          <input type="password" placeholder="Confirmar nueva contraseña" value={pwd.confirm} onChange={(e) => setPwd({ ...pwd, confirm: e.target.value })}
            className="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm" />
          {pwdError && <div className="text-sm text-red-600">{pwdError}</div>}
          {pwdOk && <div className="text-sm text-green-600">Contraseña actualizada</div>}
          <button type="submit" className="w-full py-2.5 border border-gray-300 rounded-xl text-sm font-semibold">Cambiar</button>
        </form>
      </div>

      <div className="card p-6 max-w-md">
        <h3 className="text-sm font-semibold mb-4">Cuentas de alumnos</h3>
